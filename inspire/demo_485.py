...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='RH56 RS485 demo')
    parser.add_argument('--port', default=None, help='串口设备名，例如 /dev/ttyUSB0')
    parser.add_argument('--baudrate', type=int, default=115200, help='串口波特率')
    parser.add_argument('--id', type=int, default=1, help='设备ID')
    args = parser.parse_args()

    serial_port = args.port or pick_default_port()
    if serial_port is None:
        print("未检测到可用串口，请先连接485转串口设备。")
        print("你可以手动指定端口，例如: python demo_485.py --port /dev/ttyUSB0")
        raise SystemExit(1)

    print(f"检测到/使用串口: {serial_port}")
    print('打开串口！') # 打印提示字符“打开串口”
    ser = openSerial(serial_port, args.baudrate) # 改成自己的串口号和波特率，波特率默认115200
    print('设置灵巧手运动速度参数，-1为不设置该运动速度！')
    write6(ser, args.id, 'speedSet', [1000, 1000, 1000, 1000, 1000, 1000]) # ID号改为对应灵巧手的ID，val对应的电缸ID为1,2,3,4,5,6;对应的速度值为0-1000，1000为最大值，0不运动，如果val设置为-1，相应的手指无反应
    time.sleep(1)                   # 延时1s
    print('设置灵巧手抓握力度参数！')
    write6(ser, args.id, 'forceSet', [500, 500, 500, 500, 500, 500])# ID号改为对应灵巧手的ID，val对应的电缸ID为1,2,3,4,5,6;对应的力度值为0-1000，1000为最大力，0不运动，如果val设置为-1，相应的手指无反应
    time.sleep(1)                   # 延时1s
    print('设置灵巧手运动角度参数0，-1为不设置该运动角度！')
    write6(ser, args.id, 'angleSet', [0, 0, 0, 0, 0, 0])# ID号改为对应灵巧手的ID，val对应的电缸ID为1,2,3,4,5,6;对应的角度值为0-1000，1000为最大角度，0为最小角度，如果设置为-1，相应的手指无反应
    time.sleep(1)                   # 延时1s
    print('设置灵巧手运动角度参数1000，-1为不设置该运动角度！')
    write6(ser, args.id, 'angleSet', [1000, 1000, 1000, 1000, 1000, 1000])
    
    time.sleep(1) 
    # 读取温度
    print('读取温度...')
    read6(ser, args.id, 'temp')
        
    # 读取实际角度值
    print('读取实际角度值...')
    read6(ser, args.id, 'angleAct')  
    
    print('设置灵巧手动作库序列：3！')
    writeRegister(ser, args.id, regdict['actionSeq'], 1, [3])
    time.sleep(1)                   # 延时1s
    print('运行灵巧手当前序列动作！')
    writeRegister(ser, args.id, regdict['actionRun'], 1, [1])  
